...
- (void)enterPressed {
  ...
  if (text.length > 0) {
    FIRTextMessage *message =
        [[FIRTextMessage alloc] initWithText:text
...
- (void)updateReplies {
  ...
  if (!_isLocalUser) {
    ...
    for (int i = (_messages.count <= 10 ? 0 : _messages.count - 10); i < _messages.count; i++) {
      FIRTextMessage *textMessage = _messages[i];
      textMessage = [[FIRTextMessage alloc] initWithText:textMessage.text
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  FIRTextMessage *item = _messages[indexPath.item];
...
- (void)generateChatHistoryBasic {
  ...
  _messages =
      @[
        [[FIRTextMessage alloc] initWithText:@"Hello"
                                   timestamp:date.timeIntervalSince1970
                                      userID:@""
                                 isLocalUser:YES],
        [[FIRTextMessage alloc] initWithText:@"Hey" timestamp:dateAfter userID:@"" isLocalUser:NO]
      ]
          .mutableCopy;
  [self updateReplies];
  [self.collectionView reloadData];
...
- (void)generateChatHistoryWithSensitiveContent {
  ...
  _messages =
      @[
        [[FIRTextMessage alloc] initWithText:@"Hi"
                                   timestamp:date.timeIntervalSince1970
                                      userID:@""
                                 isLocalUser:NO],
        [[FIRTextMessage alloc] initWithText:@"How are you?"
                                   timestamp:dateAfter
                                      userID:@""
                                 isLocalUser:YES],
        [[FIRTextMessage alloc] initWithText:@"My cat died"
