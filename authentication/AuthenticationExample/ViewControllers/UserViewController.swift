...
class UserViewController: UIViewController, DataSourceProviderDelegate {
    ...
    private func presentEditUserInfoController(for item: Itemable, to saveHandler: @escaping (String)->()) {
        let editController = UIAlertController(title: "Update \(item.detailTitle!)", message: nil, preferredStyle: .alert)
