...
class RemoteConfigViewController: UIViewController {
    ...
    private func updateJSONView() {
        ...
        let keys = ["recipe_name", "ingredients", "prep_time", "cook_time", "instructions", "yield", "serving_size", "notes"]
        ...
        keys.enumerated().forEach { (index, key) in
            ...
            let attributedKey = NSAttributedString(string: formattedKey, attributes: [.foregroundColor: UIColor.secondaryLabel])
            let attributedValue = NSAttributedString(string: stringValue, attributes: [.foregroundColor: UIColor.systemOrange])
