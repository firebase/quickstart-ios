...
class MainViewController: UICollectionViewController, UITextViewDelegate {
  ...
  lazy var moreAlert: UIAlertController = {
    ...
    alert.addAction(UIAlertAction(title: "Generate basic history", style: .default) { _ in
      self.generateChatHistoryBasic()
    })
    alert.addAction(UIAlertAction(title: "Generate history with sensitive content",
                                  style: .default) { _ in
      self.generateChatHistoryWithSensitiveContent()
    })
    alert.addAction(UIAlertAction(title: "Clear chat history", style: .destructive) { _ in
      self.messages = []
      self.updateReplies()
  ...
  private func updateReplies() {
    ...
    if !isLocalUser {
      ...
      for textMessage in messages.suffix(10) {
        chat.append(TextMessage(text: textMessage.text, timestamp: textMessage.timestamp,
  ...
  override func collectionView(_ collectionView: UICollectionView,
    ...
    guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell",
                                                        for: indexPath) as? MDCSelfSizingStereoCell else {
