...
class AuthenticationExampleUITests: XCTestCase {
    ...
    func testAuthAnonymously() {
        ...
        let isAnonymousCell = app.cells.containing(.staticText, identifier: "Is User Anonymous?").element
    ...
    func testAuthExistingAccount() {
        ...
        wait(forElement: app.navigationBars["User"], timeout: 5.0)
        XCTAssertTrue(app.navigationBars["User"].exists)
        XCTAssertTrue(app.staticTexts[testEmail].exists, "The user should be signed in and the email field should display their email.")
    }
    ...
    func testCreateAccountCorrectPassword() {
        ...
        wait(forElement: app.navigationBars["User"], timeout: 5.0)
        XCTAssertTrue(app.navigationBars["User"].exists)
        XCTAssertTrue(app.staticTexts[newEmail].exists, "The user should be signed into the new account.")
    }
    ...
    func DRAFT_testGoogleSignInAndLinkAccount() {
        ...
        let firstTimeLogin = app.webViews.containing(.textField, identifier: "Email or phone").element.exists
        if firstTimeLogin {
